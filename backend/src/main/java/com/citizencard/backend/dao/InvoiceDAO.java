...
import com.citizencard.backend.model.Invoice;
...
public class InvoiceDAO {
    ...
    public Integer insert(Invoice invoice) throws SQLException {
        String sql = "INSERT INTO invoices_simple (resident_id, service_name, service_code, " +
        ...
        try (Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            stmt.setString(3, invoice.getServiceCode());
            stmt.setInt(4, invoice.getAmount());
            stmt.setString(5, invoice.getPaymentStatus() != null ? invoice.getPaymentStatus() : "PENDING");
            stmt.setDate(6, Date.valueOf(invoice.getInvoiceDate()));
    ...
    public Invoice findById(Integer id) throws SQLException {
        String sql = "SELECT * FROM invoices_simple WHERE id = ?";
        try (Connection conn = DatabaseManager.getInstance().getConnection();
    ...
    public List<Invoice> findByResidentId(Integer residentId) throws SQLException {
        ...
        String sql = "SELECT * FROM invoices_simple WHERE resident_id = ? ORDER BY invoice_date DESC";
        try (Connection conn = DatabaseManager.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public List<Invoice> findPendingByResidentId(Integer residentId) throws SQLException {
        List<Invoice> invoices = new ArrayList<>();
        String sql = "SELECT * FROM invoices_simple WHERE resident_id = ? AND payment_status = 'PENDING' ORDER BY invoice_date DESC";
        try (Connection conn = DatabaseManager.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, residentId);
            try (ResultSet rs = stmt.executeQuery()) {
    ...
    public List<Invoice> findAll() throws SQLException {
        List<Invoice> invoices = new ArrayList<>();
        String sql = "SELECT * FROM invoices_simple ORDER BY invoice_date DESC";
        try (Connection conn = DatabaseManager.getInstance().getConnection();
    ...
    public void updatePaymentStatus(Integer id, String status) throws SQLException {
        String sql = "UPDATE invoices_simple SET payment_status = ?, payment_date = CURRENT_TIMESTAMP WHERE id = ?";
        try (Connection conn = DatabaseManager.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    private Invoice mapResultSetToInvoice(ResultSet rs) throws SQLException {
        ...
        invoice.setServiceCode(rs.getString("service_code"));
