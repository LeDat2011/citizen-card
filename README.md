...
> **Version 3.0** - Bảo mật nâng cao với PBKDF2-HMAC-SHA1 và kiến trúc PIN Key + Master Key
...
| Mã hóa | AES-128 ECB, RSA-1024, PBKDF2-HMAC-SHA1 |
| Key Derivation | PBKDF2 (1000 iterations, on-card) |
...
### 🔐 Bảo mật thẻ (v3.0)
- **PBKDF2-HMAC-SHA1**: PIN Key được sinh trực tiếp trên thẻ (1000 iterations)
- **Kiến trúc Master Key**: Dữ liệu được mã hóa bằng Master Key ngẫu nhiên, không phải PIN Key
- **Mã hóa AES-128**: Tất cả dữ liệu nhạy cảm được mã hóa với Master Key
- **Chữ ký RSA-1024**: Ký số cho các giao dịch quan trọng
- **Xác thực PIN**: Tối đa 5 lần thử, sau đó khóa thẻ
- **Đổi PIN nhanh**: Chỉ cần re-encrypt Master Key, không re-encrypt toàn bộ data
...
└─────────────────────────────────────────────────────────────┘
                           ...
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                   JavaCard Applet v3.0                       │
│  ┌─────────────────────────────────────────────────────┐    │
│  │                 citizen_applet                       │    │
│  │                                                       │    │
│  │  ╔═══════════════════ CRYPTO LAYER ════════════════╗ │    │
│  │  ║  ┌──────────┐    ┌───────────┐   ┌───────────┐  ║ │    │
│  │  ║  │ PBKDF2   │───▶│  PIN Key  │──▶│Master Key │  ║ │    │
│  │  ║  │ HMAC-SHA1│    │ (16 bytes)│   │ (Random)  │  ║ │    │
│  │  ║  └──────────┘    └───────────┘   └───────────┘  ║ │    │
│  │  ╚═════════════════════════════════════════════════╝ │    │
│  │                                                       │    │
│  │  ╔═══════════════ ENCRYPTED DATA ══════════════════╗ │    │
│  │  ║  ┌───────────┐ ┌───────────┐ ┌───────────────┐  ║ │    │
│  │  ║  │  Balance  │ │  Avatar   │ │ Personal Info │  ║ │    │
│  │  ║  │(AES-128)  │ │  (15KB)   │ │   (512B)      │  ║ │    │
│  │  ║  └───────────┘ └───────────┘ └───────────────┘  ║ │    │
│  │  ╚═════════════════════════════════════════════════╝ │    │
│  │                                                       │    │
...
### Luồng bảo mật PIN Key + Master Key

```
┌─────────────┐     ┌─────────────────────┐     ┌─────────────────┐
│  User PIN   │────▶│  PBKDF2-HMAC-SHA1   │────▶│    PIN Key      │
│  (4 digits) │     │  Salt: cardId       │     │   (16 bytes)    │
└─────────────┘     │  Iterations: 1000   │     └────────┬────────┘
                    └─────────────────────┘              │
                                                         ▼
┌─────────────────────────────────────────────────────────────────┐
│                    AES-128 Decrypt                               │
│         encryptedMasterKey ──▶ Master Key (plaintext)           │
└─────────────────────────────────────────────────────────────────┘
                                                         │
                                                         ▼
                    ┌─────────────────────┐     ┌─────────────────┐
                    │     Master Key      │────▶│  Decrypt Data   │
                    │   (Random 128-bit)  │     │ Balance/Info/   │
                    └─────────────────────┘     │ Avatar          │
                                                └─────────────────┘
```
...
| **Master Key** | Key ngẫu nhiên 128-bit, mã hóa dữ liệu |
...
|---------|--------|--------|
| PIN | Smart Card | PBKDF2-HMAC-SHA1 → PIN Key |
| Master Key | Smart Card | AES-128 (encrypted by PIN Key) |
| Private Key | Smart Card | Hardware-protected, không export |
| Balance | Smart Card | AES-128 (Master Key) |
| Personal Info | Smart Card | AES-128 (Master Key) |
| Avatar | Smart Card | AES-128 (Master Key) |
| Card Registry | H2 Database | Local only |
| Transaction Logs | H2 Database | Local only |

### Ưu điểm kiến trúc v3.0
- ✅ **PBKDF2 on-card**: PIN không bao giờ rời thẻ dưới dạng plaintext
- ✅ **Master Key separation**: Đổi PIN không cần re-encrypt toàn bộ data
- ✅ **Salt = cardId**: Mỗi thẻ có key khác nhau dù cùng PIN
- ✅ **Brute-force protection**: PBKDF2 + giới hạn 5 lần thử
...
### Giới hạn kỹ thuật
- **Photo size**: Tối đa 15KB (Extended APDU support)
- **Personal info**: Tối đa 512 bytes
- **APDU data**: Extended APDU support (chunked transfer cho photo)
- **RSA key**: 1024-bit (giới hạn JavaCard 2.2.1)
- **PBKDF2 iterations**: 1000 (tối ưu cho JavaCard performance)
...
**Citizen Card Management System v3.0** | Dự án học tập về JavaCard và Smart Card Development
