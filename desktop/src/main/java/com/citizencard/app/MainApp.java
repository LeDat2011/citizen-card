...
public class MainApp extends Application {
    ...
    public void start(Stage primaryStage) {
        try {
            ...
            // Lưu ý: RealCardClient sẽ quét và kết nối với JCIDE terminal khi cần (lazy
            // connection)
            service = new CitizenCardService();

            System.out.println("[OK] Service initialized (Local mode - no HTTP server needed)");
            System.out.println("[INFO] JCIDE must be running with terminal open to use card");
